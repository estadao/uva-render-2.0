<script>
    ...
    for (const resposta of respostas) {
        if (resposta.type === "minScore") {
            answersArray.minScore = parseInt(resposta.value);
        } else if (resposta.type === "maxScore") {
            answersArray.maxScore = parseInt(resposta.value);
        } else if (resposta.type === "resposta") {
            answersArray.resposta = resposta.value;
        }

        if (answersArray.resposta) {
            ranges.push(answersArray);
            answersArray = {};
        }
    }
...
<div class='uva-quest-container'>
    ...
    {:else}
        ...
        <div class='uva-quest-button-container'>
            <button class='uva-quest-button' on:click={() => restartQuiz()}>{botão2}</button>
        </div>
    {/if}
</div>
